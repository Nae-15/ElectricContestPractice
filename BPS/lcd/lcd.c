...
void LCD_ShowIntNum(u16 x,u16 y,int16_t num,u8 len,u16 fc,u16 bc,u8 sizey)
{
	...
	if(num<0)
	{
		...
		if((len-1)>0)
		{
			...
			for(t=0;t<len-1;t++)
			{
				temp=(abs_num/mypow(10,len-t-2))%10;
				if(enshow==0&&t<(len-2))
				{
					if(temp==0)
					{
						LCD_ShowChar(x+(t+1)*sizex,y,' ',fc,bc,sizey,0);
						continue;
					}else enshow=1;
				}
				LCD_ShowChar(x+(t+1)*sizex,y,temp+48,fc,bc,sizey,0);
			}
		}
	}else{
		abs_num = num;
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void LCD_ShowBinNum(u16 x,u16 y,u8 num,u16 fc,u16 bc,u8 sizey)
{         	
	u8 t,temp;
	u8 sizex=sizey/2;
	for(t=0;t<8;t++)
